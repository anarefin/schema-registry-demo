...
/**
 * Declares the {@code events.customers} domain's AMQP exchanges (main, DLX, retry) — the
 * publisher's half of the split topology.
 *
 * <p><strong>Opt-in, not auto-configured.</strong> Exchange ownership follows domain cardinality:
 * only the single service that <em>publishes</em> the customers domain declares its exchanges. That
