 ...
 * {@code compile} and emits one {@code @Configuration} of explicit {@code @Bean TypeMapping} methods
 * per module — so {@link com.example.amqp.topology.mapping.TypeMapping} registration is known at
 * build time with zero runtime reflection.
 *
 ...
 * kit's {@code Mappings}/{@code TypeMapping} and Spring's {@code @Configuration}/{@code @Bean}/
 ...
 * {@code (groupId, artifactId)} or bean name; on any such error nothing is emitted.
 */
...
public final class EventMappingProcessor extends AbstractProcessor {
    ...
    /** Simple name of the emitted {@code @Configuration} class. */
    ...
    private void generate() {
        ...
        String source = render(packageName, descriptors);
        write(packageName + "." + GENERATED_SIMPLE_NAME, source);
    }
    ...
    /**
     * The generated {@code @Configuration} lives in {@code <eventPackage>.topology}, where
