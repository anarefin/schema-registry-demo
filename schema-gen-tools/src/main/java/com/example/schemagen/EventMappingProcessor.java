...
public final class EventMappingProcessor extends AbstractProcessor {
    ...
    private void generate() {
        ...
        descriptors.sort(Comparator.comparing(MappingDescriptor::getTypeFqcn));
        ...
        String topologyFqcn = topologyPackage + "." + domain.getPublisherTopologySimpleName();
    ...
    private DomainDescriptor domainValidation(List<MappingDescriptor> descriptors) {
        ...
        for (MappingDescriptor d : descriptors) {
            if (!d.getGroupId().equals(first.getGroupId())) {
                error("Mixed @EventMapping.groupId within one contracts module: '" + first.getGroupId()
                        + "' (" + first.getTypeFqcn() + ") vs '" + d.getGroupId() + "' (" + d.getTypeFqcn()
                        + ") — a contracts module represents exactly one domain and must share a "
                        + "single groupId", d.element());
                failed = true;
            }
            if (!d.getExchange().equals(first.getExchange())) {
                error("Mixed @EventMapping.exchange within one contracts module: '"
                        + first.getExchange() + "' (" + first.getTypeFqcn() + ") vs '" + d.getExchange()
                        + "' (" + d.getTypeFqcn() + ") — a contracts module represents exactly one "
    ...
    /**
     * The package shared by every {@code @EventMapping} event class in the module (all contract events
    ...
    static String renderMappings(String packageName, List<MappingDescriptor> descriptors) {
        ...
        sb.append(" * {@code @EventMapping} classes. Do not edit — regenerated on every compile.\n");
        ...
        for (MappingDescriptor d : descriptors) {
            sb.append('\n');
            sb.append("    @Bean\n");
            sb.append("    @ConditionalOnMissingBean(name = \"").append(d.getBeanName()).append("\")\n");
            sb.append("    public TypeMapping ").append(d.getBeanName()).append("() {\n");
            sb.append("        return Mappings.forDomain(\"").append(d.getGroupId()).append("\", \"")
                    .append(d.getExchange()).append("\")\n");
            sb.append("                .json(").append(d.getTypeFqcn()).append(".class, \"")
                    .append(d.getArtifactId()).append("\", \"").append(d.getRoutingKey()).append("\");\n");
            sb.append("    }\n");
        }
        sb.append("}\n");
        return sb.toString();
    ...
    static String renderPublisherTopology(String packageName, DomainDescriptor domain) {
        String exchangeBean = domain.getBeanPrefix() + "Exchange";
        String dlxBean = domain.getBeanPrefix() + "Dlx";
        String retryExchangeBean = domain.getBeanPrefix() + "RetryExchange";
        ...
        sb.append("public class ").append(domain.getPublisherTopologySimpleName()).append(" {\n\n");
        sb.append("    private static final DomainExchanges EX = DomainTopology.of(\"")
                .append(domain.getExchange()).append("\");\n");

        appendExchangeBean(sb, exchangeBean, "getMain");
        appendExchangeBean(sb, dlxBean, "getDlx");
        appendExchangeBean(sb, retryExchangeBean, "getRetry");
     ...
     */
    static final class DomainDescriptor {

        private final String groupId;
        private final String exchange;
        private final String beanPrefix;
        private final String publisherTopologySimpleName;
        ...
        DomainDescriptor(
            ...
            this.publisherTopologySimpleName = publisherTopologySimpleName;
        }
        ...
        String getPublisherTopologySimpleName() {
            return publisherTopologySimpleName;
        }
    }
}
