...
**Status:** Accepted  
**Date:** 2026-07-15 (amended 2026-07-22 — publisher topology is now generated, not hand-written)  
**Spec:** `spec/11-separate-publisher-consumer-topology-ownership.md`,
`spec/16-build-generated-publisher-topology.md`  
**Related findings:** ARCH-009  
**Supersedes:** the exchange-ownership half of ADR-0007 ("contracts own exchanges")  
**Amended by:** [ADR-0011](ADR-0011-build-generated-type-mapping-config.md) — the `*PublisherTopology`
class this ADR introduces is, since the amendment, generated by `EventMappingProcessor` rather than
hand-written; the ownership/opt-in decision below is unchanged
...
the classpath now forces **no** exchanges. The `TypeMapping` beans (build-generated
`GeneratedEventTypeMappings`, [ADR-0011](ADR-0011-build-generated-type-mapping-config.md)) stay
auto-loaded — plain data both roles need.

**Since ADR-0011's amendment,** `*PublisherTopology` (`OrdersPublisherTopology` /
`CustomersPublisherTopology`) is itself generated — `EventMappingProcessor` emits it from the same
`@EventMapping` data it reads for `GeneratedEventTypeMappings`, into `<eventPackage>.topology`, with
no hand-written source left in the module. This ADR's opt-in ownership decision is exactly what that
generated class implements: it is deliberately withheld from `AutoConfiguration.imports`, so
loading a contracts jar alone still forces no exchanges — only the explicit
`@Import(OrdersPublisherTopology.class)` on the domain's single publisher does.
...
`producer-service`, the sole publisher of both domains, `@Import`s both `OrdersPublisherTopology`
and `CustomersPublisherTopology` (the generated classes described above), declaring all six
exchanges at startup. `consumer-service` production code is unchanged: it imports no topology and
binds its queues to the publisher-owned exchanges. (Its Testcontainers ITs import a
`*PublisherTopology` / tiny test config so the exchanges exist for the bindings.)
