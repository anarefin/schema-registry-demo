...
1. Both depend on the same `*-contracts` module (e.g. `order-contracts`), which is the
   single source of truth for an event's shape (an immutable Java **class**) and its generated
   JSON Schema.
...
3. Schema evolution is gated in CI: an incompatible change to an event class fails the build
...
graph LR
    ...
    oc["order-contracts<br/>(classes + schemas + topology + TypeMapping autoconfig)"]
    cc["customer-contracts<br/>(classes + schemas + topology + TypeMapping autoconfig)"]
...
1. **Schema generation** (victools) — invoked at `generateSchemas` via `exec-maven-plugin` (no
   Maven dependency on any contracts module). It reads compiled `@GenerateSchema`-annotated classes
...
(ADR-0011). The immutable Java class, not the schema, is authored by hand.

### Contracts

`order-contracts` and `customer-contracts` each own three things for their domain: the
event classes (`@GenerateSchema` + `@EventMapping`), the generated JSON Schemas, and two
build-generated sources emitted by the same `EventMappingProcessor` pass at `compile`
(ADR-0011) — an auto-loaded `GeneratedEventTypeMappings` `@AutoConfiguration` of `@Bean TypeMapping`
methods (no runtime reflection or index read), and an **opt-in** `@Configuration` class
(`OrdersPublisherTopology` / `CustomersPublisherTopology`) that declares that domain's three
**exchanges** (main / DLX / retry). Neither module has hand-written topology or mapping source left
— both are generated into `<eventPackage>.topology`.

Exchange ownership follows domain cardinality: only the **single service that publishes** a domain
declares its exchanges, by `@Import`-ing that domain's generated `*PublisherTopology`. These classes
are deliberately **not** in `AutoConfiguration.imports`, so a contracts jar on the classpath no
longer forces any service to declare exchanges. Consumers declare only their private *queues* and
*bind* to the publisher-owned exchanges — those per-service queues, DLQs, and retry ladders are
declared in `schema-messaging-core` by `ServiceQueueTopologyAutoConfiguration` from the
`@BitsEventHandler` scan (see Services below and §4.5).
...
`producer-service`, as the sole publisher of both domains, `@Import`s both generated
`*PublisherTopology` classes (`OrdersPublisherTopology`, `CustomersPublisherTopology`) on its
`@SpringBootApplication` and so declares all six exchanges at startup. `consumer-service` imports
no topology in production code — its queues are declared by
...
| `OrdersPublisherTopology` | generated into `order-contracts`' `<eventPackage>.topology` | Build-generated by `EventMappingProcessor` (ADR-0011), same as `GeneratedEventTypeMappings`. **Opt-in** (`@Import`-ed by the publisher, not auto-loaded). Declares the `events.orders.*` **exchanges** only (main / DLX / retry). The queues are declared per-service by `ServiceQueueTopologyAutoConfiguration`. |
| `@EventMapping` | `event-contract-kit` | Annotation on each event class (schema identity + AMQP route); read at build time by `EventMappingProcessor` to emit the mapping beans. |
| `GeneratedEventTypeMappings` | generated into `<domain>.topology` (compiled into the contracts jar) | Build-generated `@AutoConfiguration` of one `@Bean @ConditionalOnMissingBean(name=…) TypeMapping` per event class, built via `Mappings` (app override wins). Self-activates via processor-emitted `AutoConfiguration.imports`. Emitted by `schema-gen-tools`' `EventMappingProcessor` at the module's `compile`. |
...
1. HTTP request → `OrderController` builds the event instance and hands it to `EventPublisher`.
...
The controller builds the `OrderCreated` event from the request body and delegates
...
The event class itself carries the mapping metadata
...
public TypeMapping orderCreatedMapping() {
    return Mappings.forDomain("events.orders", "events.orders.exchange")
            .json(com.example.contracts.orders.OrderCreated.class, "OrderCreated", "orders.created");
}
```

`@ConditionalOnMissingBean(name=…)` means an app bean of the same name wins — no runtime
reflection, no index load. `events.mappings.include` / `exclude` are a different knob: they filter
by type identity for registry/catalog selection, not by Spring bean name. Mapping registration
never declares exchanges; publisher topology stays opt-in (ADR-0008 / ADR-0010 / ADR-0011).
...
**Exchanges (contracts, opt-in).** `OrdersPublisherTopology`, build-generated by
`EventMappingProcessor` into `order-contracts`' `com.example.contracts.orders.topology` package
(ADR-0011; not a committed source file), declares just the three `TopicExchange` beans for the
domain — `events.orders.exchange`, `.dlx`, and `.retry.exchange` — and nothing else. It is
`@Import`-ed only by the publisher (`producer-service`), not auto-loaded, so a contracts jar alone
...
sequenceDiagram
    ...
    JS-->>C: validated OrderCreated instance
...
*exactly* the same classes traced in section 4. Only the event class shape, routing key, and
...
`CustomersPublisherTopology` is the exact mirror of
`OrdersPublisherTopology` (section 4.5) for the customer domain — build-generated the same way, it
declares the customer exchanges (and is `@Import`-ed by the publisher); the queues are provisioned
per-service by `ServiceQueueTopologyAutoConfiguration` just as for orders. If you understand
section 4, you understand all seven events — the only new code for an eighth event is a new event class
in the domain event package (plus nested types if needed) with `@GenerateSchema` + `@EventMapping`
(reuse `*EventRouting` constants), a routing-key constant if needed, and one `@BitsEventHandler`
method. **No** mapping `@Bean` and **no** exchange `@Bean` to write by hand — the annotation
processor regenerates both `GeneratedEventTypeMappings` and `*PublisherTopology` at compile. Build
and commit the generated schema. There is **no** topology list to touch: the new event's
per-service queue/DLQ/retry-ladder is provisioned automatically the moment a handler for it exists,
and the domain's `*PublisherTopology` is regenerated from the module's `@EventMapping`s with no
manual edit — only a genuinely new domain needs a new `@Import` line in `producer-service`. See
README § "Adding a new event" and ADR-0011.
...
| Build-generated `TypeMapping` config (annotation processor) | [`docs/adr/ADR-0011-build-generated-type-mapping-config.md`](adr/ADR-0011-build-generated-type-mapping-config.md) |
| Annotation-driven mappings (`@EventMapping` origin; runtime index superseded) | [`docs/adr/ADR-0010-annotation-driven-event-mappings.md`](adr/ADR-0010-annotation-driven-event-mappings.md) |
