...
| **`@EventMapping`** | Declarative schema identity + AMQP route on an event record (`groupId`, `exchange`, `routingKey`; optional `artifactId` / `schemaType`). Lives in `event-contract-kit`. Never declares an exchange. Reuse `*EventRouting` constants for exchange/routing key. See ADR-0010. |
| **Event mappings index** | Build output `META-INF/event-mappings.idx` (UTF-8, one FQCN per line, lexically sorted, trailing newline). Written under `${project.build.outputDirectory}` only — **not** committed under `src/main/resources`. Regenerated every `process-classes`; packaged into the contracts jar. No runtime classpath package scan. |
...
| **TypeMapping** | Runtime wiring bean linking a Java record type → schema coordinates → schema type → RabbitMQ routing key → AMQP exchange. Lives in `event-contract-kit`, registered from the event index by each domain's `*TypeMappingAutoConfiguration` (`@RegisterEventMappings` → `EventMappingRegistrar`). Bean name = `Introspector.decapitalize(simpleName) + "Mapping"`. `EventPublisher.publish(Object event)` resolves both exchange and routing key from it — the caller supplies only the event. |
...
| **Mappings** | `event-contract-kit` domain-scoped builder: `Mappings.forDomain(group, exchange).json(javaType, routingKey)` produces a `TypeMapping` with `SchemaType.JSON` and defaults `artifactId` to the Java type's simple name. The indexed registrar (and any app-level override / test) must build mappings through it — single construction path. |
| **Bean-name override vs selection** | **Override:** define `@Bean("orderCreatedMapping")` (same name the registrar would use); registrar skips that name via `containsBeanDefinition`. **Selection:** `events.mappings.include` / `exclude` filter by simple name, FQCN, or `groupId:artifactId` — **not** Spring bean names. Different knobs. |
| **Exact-package invariant** | `@RegisterEventMappings("com.example.contracts.orders")` registers only types whose package **equals** that string (not prefix). Domain event records must live in that package. |
...
| Module | Role |
|--------|------|
| `schema-gen-tools` | Build-only schema generator (victools). Discovers `@GenerateSchema` types via package scan, requires paired `@EventMapping`, writes schemas + `META-INF/event-mappings.idx`. Never on service classpath. |
...
| `event-contract-kit` (formerly `amqp-topology-kit`) | Domain-agnostic AMQP topology-building library (naming conventions, retry-ladder factory) plus `TypeMapping`/`SchemaCoordinates`/`SchemaType`, `@GenerateSchema`, `@EventMapping`, index reader, and `EventMappingRegistrar`. Used by both `*-contracts` (exchanges + indexed mappings) and `schema-messaging-core` (per-service queues). |
| `order-contracts` / `customer-contracts` | Event records (`@GenerateSchema` + `@EventMapping`), generated schemas, opt-in publisher-owned **exchange** config (`*PublisherTopology`), and index-driven `TypeMapping` auto-config per domain. |
